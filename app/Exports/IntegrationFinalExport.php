...
class IntegrationFinalExport implements  FromQuery  ,WithHeadings
{

    use Exportable;


    public function DeadYear(int $dead_year)
    {
    ...
    //  return Integration_final ::query()->where('integration_final.DEAD_YEAR', $this->dead_year)->orderBy('PROTOCOL','ASC');

    return Integration_final ::query()->where('integration_final.DEAD_YEAR', $this->dead_year)->leftJoin('deathcert_raw', 'integration_final.id_raw_deathcert', '=', 'deathcert_raw.id')->leftJoin('eclaim_raw', 'integration_final.id_raw_eclaim', '=', 'eclaim_raw.id')->leftJoin('police_raw', 'integration_final.id_raw_police', '=', 'police_raw.id')->select('integration_final.*','deathcert_raw.FNAME','deathcert_raw.LNAME','eclaim_raw.Fname AS E_Fname','eclaim_raw.lname AS E_lname','police_raw.first_name','police_raw.last_name' )->orderBy('PROTOCOL','ASC');
