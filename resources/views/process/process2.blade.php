...
<x-app-layout>
    ...
    <div class="container mx-auto px-6 py-8">
        ...
        <form name="frm_process1" method="get" action="{{ url('process3',$masterfile_id) }}">
            @csrf
                            ...
                            <table class="w-full border">
                                ...
                                <tbody>
                                    <tr class="  border-b text-sm text-black-600">
                                        ...
                                        <td class="p-2 border-r">
                                            {{ $master_deathcert->filename ?? '' }}
                                        </td>
                                        <td class="p-2 border-r">
                                            {{ $master_deathcert->user->name ?? '' }}
                                            [{{$master_deathcert->user->department}}]
                                        </td>
                                        <td class="p-2 border-r">
                                            {{ number_format(count($data_deathcert)) ?? '' }}
                                        </td>
                                        <td class="p-2 border-r">{{ $master_deathcert->note ?? '' }}</td>
                                    ...
                                    <tr class="  border-b text-sm text-black-600">
                                        ...
                                        <td class="p-2 border-r">
                                            {{ $master_police->filename ?? '' }}
                                        </td>
                                        <td class="p-2 border-r">
                                            {{ $master_police->user->name ?? '' }}
                                            [{{$master_police->user->department}}]
                                        </td>
                                        <td class="p-2 border-r">
                                            {{ number_format(count($data_police)) ?? '' }}
                                        <td class="p-2 border-r">{{ $master_police->note ?? '' }}</td>
                                    ...
                                    <tr class="  border-b text-sm text-black-600">
                                        ...
                                        <td class="p-2 border-r">
                                            {{ $master_eclaim->filename ?? '' }}
                                        </td>
                                        <td class="p-2 border-r">
                                            {{ $master_eclaim->user->name ?? '' }}
                                            [{{$master_eclaim->user->department}}]
                                        </td>
                                        <td class="p-2 border-r">
                                            {{ number_format(count($data_eclaim)) ?? '' }}</td>
                                        <td class="p-2 border-r">{{ $master_eclaim->note ?? '' }}</td>
            ...
            <div class="flex flex-col text-center ">
                <h1>ข้อมูลที่เชื่อมต่อกันรวม : {{ $count_row ?? 0 }}</h1>
            </div>


            <div class="mt-8"></div>

            <button id="button" type="submit" onclick="return confirm('ต้องการประมวลผลตาม Protocol ใช่หรือไม่ ?');"
                class="w-full px-6 py-3 mt-3 text-lg text-white transition-all duration-150 ease-linear rounded-lg shadow outline-none bg-pink-500 hover:bg-pink-600 hover:shadow-lg focus:outline-none">
                ประมวลผลข้อมูลตาม Protocol >>
            </button>
        </form>
