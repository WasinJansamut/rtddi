...
<x-app-layout>
 ...
 $("#myDiv").toggle();
    ...
    <div class="container mx-auto px-6 py-8">
        ...
        <form name="frm_process1" method="POST" action="{{ route('process2') }}">
            <div class="flex flex-col mt-8 w-full md:w-auto item-center text-center">
                <div class='w-full md:w-full px-3 mb-6'>
                    <label
                        class='block uppercase tracking-wide text-gray-700 text-xl font-bold mb-2 inline-block'>ปีที่ต้องการประมวลผล<select
                            name="y_process"
                            class="block appearance-none text-gray-600 w-full bg-white border border-gray-400 shadow-inner px-4 py-2 pr-8 rounded"
                            OnChange="window.location='{{url('process')}}/'+this.value;">
            ...
            <div class="flex flex-col ">
                <div class="-my-2 py-2 overflow-x-auto sm:-mx-6 sm:px-6 lg:-mx-8 lg:px-8">
                    ...
                    <h4 class="text-gray-700 text-3xl font-medium">กรุณาเลือกไฟล์ที่ต้องการประมวลผล </h4>
                    <!--Card-->
                    ...
                    {{--  deathcert --}}
                    <div id='recipients' class="p-8 mt-6 lg:mt-0 rounded shadow bg-white">
                        <h4 class="text-gray-700 text-3xl font-medium"> <img
                                src="{{ asset('storage/icon/deathcert.png') }}" width="50" class="inline-block" />
                            ข้อมูลมรณบัตร
                        ...
                        <div class="table w-full p-2">
                            <table class="w-full border">
                                ...
                                <tbody>
                                    @forelse ($deathcert_master as $row_master)
                                    <tr class="bg-gray-100 text-center border-b text-sm text-gray-600">
                                        <td class="p-2 border-r">
                                            <input type="checkbox" name="check_deathcert[]" onclick="onlyOne1(this)"
                                                value="{{ $row_master->id ?? '' }}">
                                        ...
                                        <td class="p-2 border-r">{{ $row_master->filename ?? '' }}</td>
                                        <td class="p-2 border-r">{{ $row_master->death_year+543 ?? '' }}</td>
                                        <td class="p-2 border-r">{{ $row_master->err ?? '' }}</td>
                                        <td class="p-2 border-r">{{ $row_master->war ?? '' }}</td>
                                        <td class="p-2 border-r">{{ $row_master->lost ?? '' }}</td>
                                        <td class="p-2 border-r">{{ $row_master->com ?? '' }}</td>
                                        <td class="p-2 border-r"> @php // sum data count
                                            $c_prepare = App\Models\Deathcert_prepare::where([
                                            ['master_file', '=', $row_master->id]
                                            ])->count('deathcert_prepare.id');
                                            echo $c_prepare;
                                            @endphp</td>
                                        <td class="p-2 border-r">{{ $row_master->user->name ?? '' }}
                                            [{{$row_master->user->department}}]</td>
                                        <td class="p-2 border-r">{{ $row_master->note ?? '' }}</td>
                                        <td class="p-2 border-r ">

                                            <div class="flex item-center justify-center">
                                                <a href="{{url('datadeathcert',$row_master->id)}}" target="_blank">
                    ...
                    <div id='recipients' class="p-8 mt-6 lg:mt-0 rounded shadow bg-white">
                        <h4 class="text-gray-700 text-3xl font-medium"> <img
                                src="{{ asset('storage/icon/police.png') }}" width="50" class="inline-block" />
                            ข้อมูลตำรวจ
                        ...
                        <div class="table w-full p-2">
                            <table class="w-full border">
                                ...
                                <tbody>
                                    ...
                                    <tr class="bg-gray-100 text-center border-b text-sm text-gray-600">
                                        <td class="p-2 border-r">
                                            <input type="checkbox" name="check_police[]" onclick="onlyOne2(this)"
                                                value="{{ $row_master->id ?? '' }}">
                                        </td>
                                        <td class="p-2 border-r">
                                            {{ Carbon\Carbon::parse($row_master['created_at'])->addYear(543)->format('d/m/Y H:i:s') }}
                                        ...
                                        <td class="p-2 border-r">
                                            <div class="flex item-center justify-center">
                                                <a href="{{url('datapolice',$row_master->id)}}" target="_blank">
                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none"
                                                        viewBox="0 0 24 24" width="24" height="24"
                                                        stroke="currentColor">
                                                        <path stroke-linecap="round" stroke-linejoin="round"
                    ...
                    {{--  Eclaim --}}
                    <div id='recipients' class="p-8 mt-6 lg:mt-0 rounded shadow bg-white">
                        <h4 class="text-gray-700 text-3xl font-medium"> <img src="{{ asset('storage/icon/claim.png') }}"
                                width="45" class="inline-block" /> ข้อมูล บ.กลาง
                        ...
                        <div class="table w-full p-2">
                            <table class="w-full border">
                                ...
                                <tbody>
                                    @forelse ($eclaim_master as $row_master)
                                    <tr class="bg-gray-100 text-center border-b text-sm text-gray-600">
                                        <td class="p-2 border-r">
                                            <input type="checkbox" name="check_eclaim[]" onclick="onlyOne3(this)"
                                                value="{{ $row_master->id ?? '' }}">
                                        </td>
                                        <td class="p-2 border-r">
                                            {{ Carbon\Carbon::parse($row_master['created_at'])->addYear(543)->format('d/m/Y H:i:s') }}
                                        ...
                                        <td class="p-2 border-r">
                                            @php // sum data count
                                            $c_prepare = App\Models\Eclaim_prepare::where([
                                            ['master_file', '=', $row_master->id]
                                            ])->count('eclaim_prepare.id');
                                            echo $c_prepare;
                                            @endphp
                                        </td>
                                        <td class="p-2 border-r">{{ $row_master->user->name ?? '' }}
                                        ...
                                        <td class="p-2 border-r">
                                            <div class="flex item-center justify-center">
                                                <a href="{{url('dataeclaim',$row_master->id)}}" target="_blank">
                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none"
                                                        viewBox="0 0 24 24" width="24" height="24"
                                                        stroke="currentColor">
                                                        <path stroke-linecap="round" stroke-linejoin="round"
            ...
            <div class="editor mx-auto w-10/12 flex flex-col text-gray-800  p-4 shadow-lg max-w-2xl">
                ...
                <textarea class="description sec p-3 h-20 border border-gray-300 outline-none" spellcheck="false"
                    placeholder="หมายเหตุ" name="note"></textarea>


                <div id="loader" style="display:none; text-center:center;"><IMG SRC="{{ asset('storage/loading.gif') }}"
                        WIDTH="100" HEIGHT="100" BORDER="0" ALT=""><br></div>

            </div>



            <button id="myDiv" type="submit"
                onclick="return confirm('การประมวลผลข้อมูลอาจใช้เวลานาน ต้องการประมวลผลข้อมูลใช่หรือไม่ ?');"
                class="w-full px-6 py-3 mt-3 text-lg text-white transition-all duration-150 ease-linear rounded-lg shadow outline-none bg-pink-500 hover:bg-pink-600 hover:shadow-lg focus:outline-none">
                เชื่อมต่อข้อมูลเข้าด้วยกัน
            </button>
        </form>
