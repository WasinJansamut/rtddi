...
<table class="border-collapse w-full ">
    ...
    <tbody class="flex flex-col items-center justify-between overflow-y-scroll w-full"
        ...
        <tr class="flex w-full">
            ...
            <td class="w-full w-1/4 text-gray-800 text-center border border-b block lg:table-cell relative lg:static">
                {{ $item['อายุผู้ประสบเหตุ'] ?? "" }} </td>
            <td class="w-full  w-1/4  text-gray-800 text-center border border-b block lg:table-cell relative lg:static">

                @if ( empty($item['วันเดือนปีเกิดผู้ประสบเหตุ']))
                -
                @else
                {{substr($item['วันเดือนปีเกิดผู้ประสบเหตุ'],6,2)}} /
                {{substr($item['วันเดือนปีเกิดผู้ประสบเหตุ'],4,2)}}
                /{{ (int)substr($item['วันเดือนปีเกิดผู้ประสบเหตุ'],0,4) ?? "" }}
                @endif
            ...
            <td class="w-full  w-1/4  text-gray-800 text-center border border-b block lg:table-cell relative lg:static">
                @if ( empty($item['วันเวลาที่เกิดเหตุ']))
                -
                @else
                {{substr($item['วันเวลาที่เกิดเหตุ'],6,2)}} / {{substr($item['วันเวลาที่เกิดเหตุ'],4,2)}}
                /{{ (int)substr($item['วันเวลาที่เกิดเหตุ'],0,4) ?? "" }}
                @endif
            </td>
        </tr>
        @endforeach
